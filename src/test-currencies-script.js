...
const capital = +process.argv[5] || 100; // Amount in EUR which is the total money that can be used for trading
const interval = +process.argv[6] || 5; // from 5 to 11440, time per mins E.g. 11440 would be every 24 hours
const pairs = Object.keys(currencies).filter((p) => /loser/gim.test(currencies[p].note));
// /loser.*g1/gim
