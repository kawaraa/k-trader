...
class MyTrader extends Trader {
  ...
  async run() {
    ...
    const halfPercentage = this.priceChangePercent / 2;
    const pauseCase1 = this.lastTradeAge < (3 * 60) / 5;
    const pauseCase2 = calcPercentage(this.lastOrderBidPrice, currentPrice.askPrice) > 1;
    const shouldPause = trades.at(-1) > Math.max(halfPercentage, 3) && (pauseCase1 || pauseCase2);
    ...
    if (!positions[0] && this.capital > 0 && balance.eur >= 5) {
      ...
      if (
        ...
        const movement = findPriceMovement(
          last6HrsPricesBidPrices,
          buyOnPercent,
          Math.max(this.priceChangePercent, 2)
        );
      ...
      if (shouldBuy && safeAskBidSpread && !shouldPause) {
        this.dispatch(
          "LOG",
          `Placing BUY at ${currentPrice.askPrice} for ${this.priceChangePercent}% profit`
        );
      ...
      const profitable =
        (halfPercentage <= 3 && this.prevGainPercent > 1.5 && this.prevGainPercent < 3 && loss > 0.1) ||
        (this.prevGainPercent >= 3 && loss > 1) ||
        (this.prevGainPercent > 5 && loss > 1.5) ||
        (this.prevGainPercent > 6 && loss > 2);

      const stopLoss =
        (this.losses[0] >= 2 && this.losses[1] > this.losses[0] / 2 && this.losses[2] >= 1) ||
        (loss > 3 && findPriceMovement(last6HrsPricesBidPrices, 0.5, 1.5).includes("INCREASING"));

      // const protectPreviousProfit = trades.at(-1) > 3 && this.prevGainPercent >= 1 && loss > 0.1;
