...
class MyTrader extends Trader {
  constructor(exProvider, pair, interval, capital) {
    ...
    this.range = (36 * 60) / this.interval;
  ...
  async run() {
    ...
    const periodOf12Hrs = parseInt(this.range / 3);
    // Get data from Kraken
    const allPrices = await this.ex.prices(this.pair, this.range); // Change this to 12 hrs if 24 is not used
    const first12HrsPrices = allPrices.slice(0, periodOf12Hrs);
    const second12HrsPrices = allPrices.slice(periodOf12Hrs, periodOf12Hrs * 2);
    const last12HrsPrices = allPrices.slice(-periodOf12Hrs);
    const last6HrsPrices = allPrices.slice(-parseInt(periodOf12Hrs / 2));
    ...
    if (allPrices.length < this.range) return;
    ...
    const pauseCase1 = this.lastTradeAge < this.range / 4;
    ...
    const trend = detectTrendBasedCloses(second12HrsPrices.map((p) => p.bidPrice));
    if (!this.trends[0] && trend != "downtrend") {
      this.trends = [
        detectTrendBasedCloses(first12HrsPrices.map((p) => p.bidPrice)),
        trend,
        detectTrendBasedCloses(last12HrsPrices.map((p) => p.bidPrice)),
      ];
    }
    ...
    if (!positions[0] && this.capital > 0 && balance.eur >= 5) {
      ...
      if (this.trends[1] == "uptrend" && this.trends[2] == "downtrend") {
      ...
      //   const movement = findPriceMovement(
      //     last12HrsPrices.map((p) => p.bidPrice),
      //     Math.max(this.priceChangePercent / 5, 0.5),
      //     Math.max(this.priceChangePercent / 2, 1)
      //   );
      //   if (movement.includes("INCREASING")) {
      ...
      const profitable =
        (this.priceChangePercent / 2 <= 2 && gainLossPercent >= 2 && gainLossPercent < 3 && loss > 0.1) ||
