...
class MyTrader extends Trader {
  ...
  async run() {
    ...
    if (!pause && decision.signal.includes("BUY") && this.capital > 0 && balance.eur >= 5) {
      ...
      //
    } else if (balance.crypto > 0 && decision.signal.includes("SELL")) {
    ...
    while (percentage <= maxPercentage) {
      ...
      for (let i = range; i < prices.length; i++) {
        ...
        if (decision.signal.includes("SELL-PROFITABLE")) {
          ...
          trades++;
          price = null;
        } else if (decision.signal.includes("SELL-STOP-LOSS")) {
